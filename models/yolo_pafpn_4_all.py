...
from .darknet_all_trans import CSPDarknet_all_trans


class YOLOPAFPN_4_all(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.backbone = CSPDarknet_all(depth, width, depthwise=depthwise, act=act)
...
#####################################################################################################
    ...
    def forward(self, input):
        ...
        fpn_out2 = self.reduce_conv0(f_out1)  # 256->128/8
        f_out2 = self.upsample(fpn_out2)  # 128/4
        f_out2 = torch.cat([f_out2, x3], 1)  # 128->256/4
        pan_out3 = self.C3_p2(f_out2)  # 512->256/4
